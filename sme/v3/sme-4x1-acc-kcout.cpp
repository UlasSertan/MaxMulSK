...
namespace SMEKernels4x1AccKcOut {
    ...
    void micro_kernel_4x1(float* RESTRICT packed_A, float* RESTRICT packed_B,
    ...
    //
    // 4x1's four ZA tiles stack VERTICALLY: tile t owns C rows
    // [t*SVL, (t+1)*SVL), and each of those rows is exactly SVL wide -- one
    // vector. So unlike 1x4 (tile 4*SVL wide, one svst1_f32_x4 per row, 64
    // narrow stores collapsing to 16) and 2x2 (2*SVL wide, x2, 64 -> 32),
    // there is NOTHING to widen here. 64 rows, one narrow store each, before
    // and after. The row-major rework that carried most of 1x4-Acc's win does
    // not transfer to this geometry at all.
    //
    // What does transfer is dropping the read-modify-write: this store does not
    // read C back, saving 64 loads and 64 adds per tile.
    //
    // OVERWRITES C. Sound because K is innermost and the outer Kc loop gives the
    // first panel a tile nobody has written yet; later panels use store_za_add.
    // =========================================================================
    __attribute__((noinline))
    static void store_za(float* RESTRICT C, size_t wide_of_C) __arm_in("za") __arm_streaming {
        ...
        // Tile index must be a compile-time constant, so unroll (TODO.md).
        #define ST_4x1(TILE)                                                     \
        do {                                                                      \
            float* Ct = C + (TILE) * SVL * wide_of_C;                             \
            for (int i = 0; i < (int)SVL; i++)                                    \
                svst1_f32(pg, Ct + i * wide_of_C,                                 \
                          svread_hor_za32_f32_m(inactive, pg, TILE, i));          \
        } while (0)
        ST_4x1(0); ST_4x1(1); ST_4x1(2); ST_4x1(3);
        #undef ST_4x1
    }

    // Accumulating variant, for every K panel after the first.
    __attribute__((noinline))
    static void store_za_add(float* RESTRICT C, size_t wide_of_C) __arm_in("za") __arm_streaming {
        ...
        do {                                                                      \
            float* Ct = C + (TILE) * SVL * wide_of_C;                             \
            for (int i = 0; i < (int)SVL; i++) {                                  \
                float* ptr = Ct + i * wide_of_C;                                  \
                svfloat32_t za  = svread_hor_za32_f32_m(inactive, pg, TILE, i);   \
                svst1_f32(pg, ptr, svadd_f32_x(pg, svld1_f32(pg, ptr), za));      \
            }                                                                     \
        } while (0)
        ACC_4x1(0); ACC_4x1(1); ACC_4x1(2); ACC_4x1(3);
        #undef ACC_4x1
    }
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        // Packed panels are Kc deep, not K deep: the outer panel caps the
        // working set, which is what keeps this clear of the full-K packing
        // regression K-innermost caused in 1x4-Acc.
        ...
        for (size_t kk = 0; kk < K; kk += Kc) {
            ...
            // C = A*B, so the caller must NOT pre-zero C -- unlike the shipped
            // 4x1, which accumulates.
            const bool first_k = (kk == 0);

            // N outer then M, keeping the shipped kernel's nesting preference.
            // The acc treatment only requires K to be INNERMOST.
            for (size_t n = 0; n < N; n += N_tile) {
                ...
                for (size_t m = 0; m < M; m += M_tile) {
                    ...
                    // and the micro-kernel would read uninitialised data (its
                    // results land in discarded scratch rows, but the read
                    // itself must still be defined). Zero first -- edge strips
                    // only; aligned sizes never pay this. SVE stores, never a
                    // scalar fill, or this becomes __arm_sc_memset (BUG-5).
                    if (mc < M_tile) {
                        svbool_t pg_z = svptrue_b32();
                        svfloat32_t zero = svdup_f32(0.0f);
                        float* zp = packed_A.get();
                        for (size_t i = 0; i < kcl * M_step; i += SVL)
                    ...
                    for (size_t jr = 0; jr < nc; jr += N_step) {
                        ...
                        for (size_t ir = 0; ir < mc; ir += M_step) {
                            ...
                            if (!m_tail && !n_tail) {
                                ...
                                // store_za overwrites the whole scratch tile, so
                                ...
                                // scalar copy is lowered to __arm_sc_memcpy,
                                // which has no streaming-mode implementation.
