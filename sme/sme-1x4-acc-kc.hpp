...
// =============================================================================
// SME 1x4-Acc-Kc: 1x4-Acc with the K tile further split into Kc chunks.
//
...
// accumulates over every chunk, and stores once after the K loop.
//
// SEMANTICS: C = A*B (overwrite), NOT C += A*B. See sme-1x4-acc.hpp.
//
// Difference from 1x4-Acc: 1x4-Acc issues one micro-kernel call per K tile,
// this one splits each K tile into Kc sub-chunks and issues one call per Kc.
// Same arithmetic and same ZA lifetime; only the invocation granularity differs,
