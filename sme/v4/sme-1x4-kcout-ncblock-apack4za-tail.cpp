#include "sme-1x4-kcout-ncblock-apack4za-tail.hpp"
...
#define TL_C(g) (4 * ((g) % 4))
#define TL_GROUP(g)                                                            \
    svcreate4_f32(svread_ver_za32_f32_m(zu, pg, TL_T(g), TL_C(g) + 0),         \
                  svread_ver_za32_f32_m(zu, pg, TL_T(g), TL_C(g) + 1),         \
                  svread_ver_za32_f32_m(zu, pg, TL_T(g), TL_C(g) + 2),         \
                  svread_ver_za32_f32_m(zu, pg, TL_T(g), TL_C(g) + 3))
...
#define TL_PACK_A_KP_CHUNK(SRC, STRIDE)                                           \
    do {                                                               \
        ...
        svfloat32x4_t s0 = TL_GROUP(0);                                \
        svfloat32x4_t s1 = TL_GROUP(1);                                \
        /* Static unroll: the tile operand of svread_ver_za32 must */  \
        /* be an immediate, so g cannot be a runtime variable. */      \
        TL_DRAIN(0)  TL_DRAIN(2)  TL_DRAIN(4)  TL_DRAIN(6)             \
        TL_DRAIN(8)  TL_DRAIN(10) TL_DRAIN(12)                         \
...
namespace SMEKernels1x4NcBlockTail {
    ...
    // TEST ONLY. Expands the SAME TL_PACK_A_KP_CHUNK macro as the driver, so the
