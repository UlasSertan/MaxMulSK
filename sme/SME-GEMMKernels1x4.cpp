...
namespace SMEKernels1x4 {
    ...
    // Layout within a block per k-step: SVL contiguous floats = one column of A.
    // Micro-kernel reads one SVL-wide A vector per k-step.
    // =========================================================================
    ...
    void pack_A_streaming(const float* A, float* packed_A,
        ...
        for (size_t m = 0; m < M_curr; m += SVL) {
            ...
            size_t m_idx = m / SVL;
            // Each m-step gets its own contiguous block of K_curr * SVL floats
            ...
            for (; k < K_full; k += SVL) {
                ...
                // Store 16 cols consecutively, stride GS = SVL
            ...
            // K tail: one column at a time via tmp buffer (SVE-zeroed, no memset)
            float tmp[16];
    ...
    // B is row-major with N fast → no transpose, just reshape.
    // =========================================================================
    __attribute__((noinline))
    void pack_B_streaming(const float* B, float* packed_B,
        ...
        const size_t group_stride = K_curr * GS; // floats per 4*SVL-col block
        ...
        // N tail: remaining columns (< 4*SVL). Predicated loads zero invalid lanes.
        // Kernel still reads 4*SVL per k-step from this block; zero lanes contribute nothing.
        if (n < N_curr) {
            float* block_base = packed_B + (n / GS) * group_stride;
    ...
    // =========================================================================
    // MICRO KERNEL 1x4: SVL rows × 4*SVL cols using ZA accumulator
    // ZA_i = A ⊗ B_i  — same A panel, 4 different B panels per k-step.
    // K-unrolled by 4, software-pipelined. Per 4 k-steps:
    //   1 A x4 load (covers 4 consecutive k-steps, one A per k-step)
    //   4 B x4 loads (one per k-step, each covering 4 B panels)
    //
    // Schedule per 4 k-steps (16 svmopa + 5 x4 loads):
    //   PROLOGUE: load A x4 (k+0..k+3), load B x4 (k+0)
    //   group k+0: ZA0, ZA1, [load B x4 (k+1)], ZA2, ZA3
    //   group k+1: ZA0, ZA1, [load B x4 (k+2)], ZA2, ZA3
    //   group k+2: ZA0, ZA1, [load B x4 (k+3)], ZA2, ZA3
    //   group k+3: ZA0, ZA1, [load next A x4], ZA2, ZA3
    //   end-of-iter: load next B x4 (k+0 of next iter)
    // Gap between same-tile writes: 4 slots.
    // =========================================================================
    __attribute__((noinline))
    void micro_kernel_1x4(float* RESTRICT packed_A, float* RESTRICT packed_B, float* RESTRICT C,
        ...
        if (K_main >= 4) {
            // PROLOGUE: load first A x4 (covers k+0..k+3) and first B x4 (for k+0)
            svfloat32_t a  = svld1_f32(pg, pA); pA += SVL;
            svfloat32_t a_next = svld1_f32(pg, pA); pA += SVL;

            svfloat32_t b0  = svld1_f32(pg, pB); pB += SVL;
            svfloat32_t b1  = svld1_f32(pg, pB); pB += SVL;
            svfloat32_t b2  = svld1_f32(pg, pB); pB += SVL;
            svfloat32_t b3  = svld1_f32(pg, pB); pB += SVL;

            for (size_t k = 0; k < K_main - 4; k += 4) {
                // group k+0: interleave svget4 extracts between svmopa issues

                svmopa_za32_f32_m(0, pg, pg, a, b0);
                b0  = svld1_f32(pg, pB); pB += SVL;
                svmopa_za32_f32_m(1, pg, pg, a, b1);
                b1  = svld1_f32(pg, pB); pB += SVL;
                svmopa_za32_f32_m(2, pg, pg, a, b2);
                b2  = svld1_f32(pg, pB); pB += SVL;
                svmopa_za32_f32_m(3, pg, pg, a, b3);
                b3  = svld1_f32(pg, pB); pB += SVL;


                // group k+1
                a  = a_next;
                a_next = svld1_f32(pg, pA); pA += SVL;
                svmopa_za32_f32_m(0, pg, pg, a, b0);
                b0  = svld1_f32(pg, pB); pB += SVL;
                svmopa_za32_f32_m(1, pg, pg, a, b1);
                b1  = svld1_f32(pg, pB); pB += SVL;
                ...
                // group k+3: also prefetch next iter's a_x4 and b_x4
                a  = a_next;
                a_next = svld1_f32(pg, pA); pA += SVL;
                svmopa_za32_f32_m(0, pg, pg, a, b0);
                b0  = svld1_f32(pg, pB); pB += SVL;
                ...
                a  = a_next; // Next iterations a
                a_next = svld1_f32(pg, pA); pA += SVL;
            }

            // EPILOGUE: last group of 4 (no prefetch of next iter)
            {
                // group k+0
                svmopa_za32_f32_m(0, pg, pg, a, b0);
                b0  = svld1_f32(pg, pB); pB += SVL;
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        // Scratch for edge tiles: full M_step × N_step = SVL × 4*SVL
                    ...
                    for (size_t jr = 0; jr < nc; jr += N_step) {
                        ...
                        for (size_t ir = 0; ir < mc; ir += M_step) {
                            ...
                            if (!m_tail && !n_tail) {
                                ...
                                // Scatter valid rows × cols back into C with accumulation
