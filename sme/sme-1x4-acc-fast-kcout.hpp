...
// =============================================================================
// SME 1x4-Acc-Fast-KcOut: outer Kc loop AND pack-once-per-panel.
//
// Combines the two experiments: an outer Kc loop above M (packed buffers
// scale with Kc, C tiles revisited K/Kc times, first panel overwrites and
// the rest accumulate), plus whole-matrix packed copies within each panel
// so B is packed once per panel instead of once per (m, n) tile.
//
