...
// =============================================================================
// SME 1x4-Acc-Kc  (experimental; copied from 1x4-Acc)
//
...
namespace SMEKernels1x4AccKc {
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        // Sub-chunk of a K tile: one micro-kernel call per Kc instead of one
        // per K tile. Must stay in sync with AccChunk<true> in
        // bench/maxmulsk_sme_adapter.cpp, which replays this loop.
        constexpr size_t Kc = 2048;

        const size_t M_step = 1 * SVL;
        const size_t N_step = 4 * SVL;

        // Loop order is M -> N -> K with K innermost, so A and B are packed over
        // the FULL K before the tile loops. Buffers therefore scale with K, not
        // with K_tile.
        ...
        AlignedBuffer packed_B(static_cast<float*>(
            std::aligned_alloc(64, N_tile * K * sizeof(float))));
                ...
                for (size_t jr = 0; jr < nc; jr += N_step) {
                    ...
                    for (size_t ir = 0; ir < mc; ir += M_step) {
                        ...
                        for (size_t k = 0; k < K; k += K_tile) {
                            ...
                            for (size_t kk = 0; kk < kc; kk += Kc) {
                                size_t kchunk = std::min(Kc, kc - kk);
                                micro_kernel_1x4(
                                    packed_A.get() + ir * K + (k + kk) * SVL,
                                    packed_B.get() + jr * K + (k + kk) * N_step,
                                    kchunk);
                            }
                        }
