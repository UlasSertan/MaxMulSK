#include "sme-1x4-bstream-ilv1.hpp"
...
//
// B IS NEVER PACKED. Measured 2026-09-10 at 256^3: routing B through a memory
// staging buffer cost -51% to -73%, while dropping the packed B panel entirely
// cost nothing. B is read straight from memory at stride N.
//
// SCHEDULING, which is the point of this file:
//   * svmopa are NEVER left in long back-to-back runs. Here: never two in a row -- an svld1_f32 sits after every one.
//   * Fetches are spread across the whole group rather than bunched at its
//     head, so the load pipeline keeps working instead of going idle while a
//     run of multiplies drains.
//   * A is fetched one whole group ahead, so a group boundary is not a
//     load -> svget4 -> svmopa dependency chain.
//   * Every B load lands 12 svmopa before the svmopa that consumes it.
// Four B k-steps and two A quads stay live; see the measured spill count in
// the commit notes rather than trusting this comment.
// =============================================================================
...
namespace SMEKernels1x4BStreamIlv1 {
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        const svcount_t pg2 = svptrue_c32(); (void)pg2;
        ...
        for (size_t n = 0; n < N; n += N_step) {
            for (size_t ir = 0; ir < M; ir += M_step) {
                ...
                svzero_za();
                    if (K_main >= GRP) {
                        // PROLOGUE (pipeline fill): three k-steps of B plus the
                        // first group's A. These are the only exposed loads.
                        svfloat32_t q0_0 = svld1_f32(pg, bsrc + 0 * N + 0 * SVL);
                        svfloat32_t q0_1 = svld1_f32(pg, bsrc + 0 * N + 1 * SVL);
                        svfloat32_t q0_2 = svld1_f32(pg, bsrc + 0 * N + 2 * SVL);
                        svfloat32_t q0_3 = svld1_f32(pg, bsrc + 0 * N + 3 * SVL);
                        svfloat32_t q1_0 = svld1_f32(pg, bsrc + 1 * N + 0 * SVL);
                        svfloat32_t q1_1 = svld1_f32(pg, bsrc + 1 * N + 1 * SVL);
                        svfloat32_t q1_2 = svld1_f32(pg, bsrc + 1 * N + 2 * SVL);
                        svfloat32_t q1_3 = svld1_f32(pg, bsrc + 1 * N + 3 * SVL);
                        svfloat32_t q2_0 = svld1_f32(pg, bsrc + 2 * N + 0 * SVL);
                        svfloat32_t q2_1 = svld1_f32(pg, bsrc + 2 * N + 1 * SVL);
                        svfloat32_t q2_2 = svld1_f32(pg, bsrc + 2 * N + 2 * SVL);
                        svfloat32_t q2_3 = svld1_f32(pg, bsrc + 2 * N + 3 * SVL);
                        ...
                        svfloat32_t q3_0 = svld1_f32(pg, bsrc + 3 * N + 0 * SVL);
                        svfloat32_t q3_1 = svld1_f32(pg, bsrc + 3 * N + 1 * SVL);
                        svfloat32_t q3_2 = svld1_f32(pg, bsrc + 3 * N + 2 * SVL);
                        svfloat32_t q3_3 = svld1_f32(pg, bsrc + 3 * N + 3 * SVL);

                        // STEADY: every svmopa pair is followed by a fetch, and
                        // the fetches walk the whole group instead of bunching at
                        // its head. Each B load lands 12 svmopa before its use.
                        for (size_t k = 0; k + GRP < K_main; k += GRP) {
                            // k-step 0: consume q0, fetch k+3 into q3 (used 12 svmopa later).
                            svmopa_za32_f32_m(0, pg, pg, a0, q0_0);
                            svfloat32_t q3_0 = svld1_f32(pg, bsrc + 3 * N + 0 * SVL);
                            svmopa_za32_f32_m(1, pg, pg, a0, q0_1);
                            svfloat32_t q3_1 = svld1_f32(pg, bsrc + 3 * N + 1 * SVL);
                            svmopa_za32_f32_m(2, pg, pg, a0, q0_2);
                            svfloat32_t q3_2 = svld1_f32(pg, bsrc + 3 * N + 2 * SVL);
                            svmopa_za32_f32_m(3, pg, pg, a0, q0_3);
                            svfloat32_t q3_3 = svld1_f32(pg, bsrc + 3 * N + 3 * SVL);

                            // k-step 1: consume q1, fetch k+4 into q0 (used 12 svmopa later).
                            svmopa_za32_f32_m(0, pg, pg, a1, q1_0);
                            q0_0 = svld1_f32(pg, bsrc + 4 * N + 0 * SVL);
                            svmopa_za32_f32_m(1, pg, pg, a1, q1_1);
                            q0_1 = svld1_f32(pg, bsrc + 4 * N + 1 * SVL);
                            svmopa_za32_f32_m(2, pg, pg, a1, q1_2);
                            q0_2 = svld1_f32(pg, bsrc + 4 * N + 2 * SVL);
                            svmopa_za32_f32_m(3, pg, pg, a1, q1_3);
                            q0_3 = svld1_f32(pg, bsrc + 4 * N + 3 * SVL);

                            // k-step 2: consume q2, fetch k+5 into q1 (used 12 svmopa later).
                            svmopa_za32_f32_m(0, pg, pg, a2, q2_0);
                            q1_0 = svld1_f32(pg, bsrc + 5 * N + 0 * SVL);
                            svmopa_za32_f32_m(1, pg, pg, a2, q2_1);
                            q1_1 = svld1_f32(pg, bsrc + 5 * N + 1 * SVL);
                            // A for the NEXT group, pulled a whole group early so the
                            // group boundary is not a load -> svget4 -> svmopa chain.
                            a_nxt = svld1_f32_x4(pg4, pA); pA += GRP * SVL;
                            svmopa_za32_f32_m(2, pg, pg, a2, q2_2);
                            q1_2 = svld1_f32(pg, bsrc + 5 * N + 2 * SVL);
                            svmopa_za32_f32_m(3, pg, pg, a2, q2_3);
                            q1_3 = svld1_f32(pg, bsrc + 5 * N + 3 * SVL);

                            // k-step 3: consume q3, fetch k+6 into q2 (used 12 svmopa later).
                            svmopa_za32_f32_m(0, pg, pg, a3, q3_0);
                            q2_0 = svld1_f32(pg, bsrc + 6 * N + 0 * SVL);
                            svmopa_za32_f32_m(1, pg, pg, a3, q3_1);
                            q2_1 = svld1_f32(pg, bsrc + 6 * N + 1 * SVL);
                            svmopa_za32_f32_m(2, pg, pg, a3, q3_2);
                            q2_2 = svld1_f32(pg, bsrc + 6 * N + 2 * SVL);
                            svmopa_za32_f32_m(3, pg, pg, a3, q3_3);
                            q2_3 = svld1_f32(pg, bsrc + 6 * N + 3 * SVL);
                        ...
                        // EPILOGUE: last group. The steady loop only ever refills
                        // q3 for the group it is running, so the final group's q3
                        // is still four k-steps stale here and must be refetched.
                        // Nothing beyond it to prefetch, so this is the one place
                        // svmopa run back to back -- once per tile out of K/GRP.
                        q3_0 = svld1_f32(pg, bsrc + 3 * N + 0 * SVL);
                        q3_1 = svld1_f32(pg, bsrc + 3 * N + 1 * SVL);
                        q3_2 = svld1_f32(pg, bsrc + 3 * N + 2 * SVL);
                        q3_3 = svld1_f32(pg, bsrc + 3 * N + 3 * SVL);
                        svmopa_za32_f32_m(0, pg, pg, a0, q0_0);
                        svmopa_za32_f32_m(1, pg, pg, a0, q0_1);
                        svmopa_za32_f32_m(2, pg, pg, a0, q0_2);
                        svmopa_za32_f32_m(3, pg, pg, a0, q0_3);
