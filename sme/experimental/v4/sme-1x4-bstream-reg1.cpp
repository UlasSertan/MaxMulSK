#include "sme-1x4-bstream-reg1.hpp"
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        for (size_t n = 0; n < N; n += N_step) {
            for (size_t ir = 0; ir < M; ir += M_step) {
                ...
                svzero_za();
                    // Depth 1: every B quad is loaded immediately before the four
                    // svmopa that consume it. No look-ahead, no staging buffer.
                    // This is the control the deeper window is measured against.
                    for (size_t k = 0; k < K_main; k += GRP) {
                        ...
                        // k-step 0: load, then consume straight away.
                        ...
                        // k-step 1: load, then consume straight away.
                        ...
                        // k-step 2: load, then consume straight away.
                        ...
                        // k-step 3: load, then consume straight away.
