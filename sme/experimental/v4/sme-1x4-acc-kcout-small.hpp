...
//
// The K panel loop, the M tile loop and the edge-tile path have all been
// removed while the design is being worked out. What is left is: pack A once,
// then per N block pack B, run the micro-kernel over K, and store.
//
// CONSEQUENCE: correct only when M is a multiple of M_step (SVL) and N is a
// multiple of N_step (4*SVL). Anything else writes partial tiles at full width
// and is silently wrong. The tail handling is coming back.
// =============================================================================
...
namespace SMEKernels1x4AccKcOutSmall {
    ...
    // Streaming implementation with the blocking passed in. Exposed so
    // benchmarks and parameter sweeps can drive the knobs directly instead of
    ...
    // Single-buffered variant of the same kernel: the staging buffer is filled
    // completely, then consumed, so producer and consumer never overlap. Exists
    // as the measured control for the double-buffered default above.
    void run_multiplication_buf1(const float* A, const float* B, float* C,
                                 size_t M, size_t K, size_t N);

    // Control with no staging buffer: B is read straight from memory at stride
    // N into the fmopa. Same tile walk and same B traffic as buf1/default.
