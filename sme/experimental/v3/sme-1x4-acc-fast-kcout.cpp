#include "sme-1x4-acc-fast-kcout.hpp"
...
namespace SMEKernels1x4AccFastKcOut {
    ...
    void run_multiplication(const float* A, const float* B, float* C,
        ...
        // Whole-matrix packed copies, but only Kc deep: within one K panel each
        // row block of A and column group of B is packed once and then reused by
        // every m / n tile that needs it, instead of being repacked per tile.
        // Rounded up to the packers' block widths for the same reason as in
        // 1x4-Acc-Fast -- the last block is always consumed whole.
        ...
        AlignedBuffer packed_A(static_cast<float*>(
            std::aligned_alloc(64, M_padded * Kc * sizeof(float))));
        AlignedBuffer packed_B(static_cast<float*>(
            std::aligned_alloc(64, N_padded * Kc * sizeof(float))));
        ...
        for (size_t kk = 0; kk < K; kk += Kc) {
            ...
            // Reset per K panel: a new slice of K means both packed copies are
            // stale, so everything must be packed again for this panel.
            ...
            for (size_t m = 0; m < M; m += M_tile) {
                ...
                // Slot for this m tile. Blocks are SVL x kcl here, so the
                // offset of global block m/SVL is m * kcl.
                if (first_pack_A)
                    pack_A_streaming(A, packed_A.get() + m * kcl, mc, kcl, m, kk, K);

                for (size_t n = 0; n < N; n += N_tile) {
                    size_t nc = std::min(N_tile, N - n);

                    // Same on the B side with group width N_step = 4*SVL.
                    if (first_pack_B)
                        pack_B_streaming(B, packed_B.get() + n * kcl, nc, kcl, kk, N, n);
                    ...
                    for (size_t jr = 0; jr < nc; jr += N_step) {
                        ...
                        for (size_t ir = 0; ir < mc; ir += M_step) {
                            ...
                            for (size_t k = 0; k < kcl; k += K_tile) {
                                ...
                                micro_kernel_1x4(
                                    packed_A.get() + (m + ir) * kcl + k * SVL,
                                    packed_B.get() + (n + jr) * kcl + k * N_step,
                                    kc);
                            }
                ...
                // One full n pass has filled every B group for this K panel.
                // A is not cleared: each m iteration packs a different, still
                // empty slice of A within this panel.
                first_pack_B = false;
            }
        }
    }
