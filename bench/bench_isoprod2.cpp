// =============================================================================
// The 2^25 constant-product family with Nc DOUBLED, Mc and Kc left alone.
//
// Every entry is one point of bench_isoprod.cpp with Nc x2, so the product is
// 2^26 and packed B is twice as large. That pushes its ceiling from 8 MiB to
// 16 MiB, which is exactly this machine's P-core L2 (hw.perflevel0.l2cachesize).
// The question this run asks is whether leaning that much harder on L2 breaks
// anything; 4 of the 42 points sit exactly at the 16 MiB ceiling, and Nc now
// reaches 8192.
//
// Ratios are against v4a, the same baseline bench_isoprod.cpp used, so the two
// runs' ratio columns can be read side by side even though they are different
// binaries.
//
...
int main(int argc, char** argv) {
    ...
    for (int si = 0; si < kNShapes; si++) {
        ...
        for (int i = 0; i < NC; i++) {
            ...
            const bool onsurf = ((size_t)b.Mc*b.Nc*b.Kc) == ((size_t)1<<26);
