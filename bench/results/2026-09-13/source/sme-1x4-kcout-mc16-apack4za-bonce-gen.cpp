#include "sme-1x4-kcout-mc16-apack4za-bonce-gen.hpp"
...
// =============================================================================
// EXPERIMENT F2-GEN -- F2 generalised off its single target shape
//
...
#define F2_C(g) (4 * ((g) % 4))
#define F2_GROUP(g)                                                            \
    svcreate4_f32(svread_ver_za32_f32_m(zu, pg, F2_T(g), F2_C(g) + 0),         \
                  svread_ver_za32_f32_m(zu, pg, F2_T(g), F2_C(g) + 1),         \
                  svread_ver_za32_f32_m(zu, pg, F2_T(g), F2_C(g) + 2),         \
                  svread_ver_za32_f32_m(zu, pg, F2_T(g), F2_C(g) + 3))
...
#define F2_PACK_A_KP_CHUNK()                                           \
    do {                                                               \
        ...
        svfloat32x4_t s0 = F2_GROUP(0);                                \
        svfloat32x4_t s1 = F2_GROUP(1);                                \
        /* Static unroll: the tile operand of svread_ver_za32 must */  \
        /* be an immediate, so g cannot be a runtime variable. */      \
        F2_DRAIN(0)  F2_DRAIN(2)  F2_DRAIN(4)  F2_DRAIN(6)             \
        F2_DRAIN(8)  F2_DRAIN(10) F2_DRAIN(12)                         \
...
namespace SMEKernels1x4KcOutMc16Apack4ZaBonceGen {
    ...
    static bool run_f2_streaming(const float* A, const float* B, float* C,
                                 ...
                                 float* packed_A, float* packed_B, size_t Kc_max) {
        ...
        for (size_t kk = 0; kk < K; kk += Kc_max) {
            const bool first_k = (kk == 0);
            // The real length of THIS slice. Every consumer below -- A packing,
            // B packing and the micro-kernel -- is given this same value; a
            // mismatch would pack one length and multiply another.
            const size_t kc = std::min(Kc_max, K - kk);

            for (size_t m = 0; m < M; m += Mc) {
                for (size_t n = 0; n < N; n += Nc) {
                    const size_t panel_id = n / Nc;
                    // Panel stride is the ALLOCATED depth, not this slice's
                    // depth, so a short final slice leaves a gap rather than
                    // shifting every later panel.
                    float* const pB = packed_B + panel_id * (Kc_max * Nc);
    ...
    // TEST ONLY. Expands the SAME F2_PACK_A_KP_CHUNK macro as the driver, so
    ...
    bool probe_pack_A(const float* A, size_t M, size_t K,
        ...
        if (static_cast<size_t>(svcntsw()) != 16) return false;
        if (m + 16 > M || kk + 2048 > K) return false;   // probe is fixed at Kc=2048
        const svbool_t  pg = svptrue_b32();
        const svcount_t p4 = svptrue_c32();
        const svfloat32_t zu = svundef_f32();
    ...
    // Public entry. Deliberately NOT streaming: the capability check and the
    // allocation belong outside the streaming region, which is entered once.
    //
    // This kernel has NO tail path. It runs natively only when every tile is
    // full, which needs all three of:
    //   M % 16 == 0   one A panel is exactly 16 rows
    //   N % 64 == 0   store_za writes a full 16x64 tile
    //   K % 64 == 0   the inline A packing consumes k in chunks of 64
    // Anything else returns Unsupported and leaves C untouched. It does NOT
    // fall back internally: choosing another kernel is the caller's decision,
    // and a fallback result is not this kernel's performance.
    ...
    // Packed working set for a shape, so a sweep can report it without running.
    void capacity(size_t K, size_t N, size_t* packed_A_bytes, size_t* packed_B_bytes) {
        const size_t Kc_max = std::min<size_t>(2048, K);
        const size_t panels = (N + 63) / 64;
        *packed_A_bytes = 16 * Kc_max * sizeof(float);
        *packed_B_bytes = panels * Kc_max * 64 * sizeof(float);
    }

    Support run_multiplication(const float* A, const float* B, float* C,
                               size_t M, size_t K, size_t N) {
        const Support s = classify(M, K, N);
        if (s != Support::Native) return s;

        // Kc is capped at 2048 but never exceeds K, so a short K is one slice.
        // packed_B holds every N panel for one Kc slice -- that is the whole
        // point of packing B once per slice, and it means the buffer grows
        // linearly with N. At N = 32768 and Kc = 512 that is 64 MiB.
        const size_t Kc_max = std::min<size_t>(2048, K);
        const size_t panels = N / 64;
        AlignedBuffer packed_A(static_cast<float*>(
            std::aligned_alloc(64, 16 * Kc_max * sizeof(float))));
        AlignedBuffer packed_B(static_cast<float*>(
            std::aligned_alloc(64, panels * Kc_max * 64 * sizeof(float))));
        if (!packed_A || !packed_B) return Support::AllocationFailed;

        return run_f2_streaming(A, B, C, M, K, N, packed_A.get(), packed_B.get(), Kc_max)
